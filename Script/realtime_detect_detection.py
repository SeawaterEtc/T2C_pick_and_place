...
# Load model
model = YOLO("T2C_PickAndPlace/Data/AImodel/detect40.pt")  # segmentation model
...
def draw_bounding_box(image, box, cls, color=(0, 255, 0), thickness=2):
    x, y, w, h, angle = box
    center = (int(x), int(y))
    size = (int(w), int(h))
...
while True:
    ...
    if results[0].boxes is not None:
        ...
        for box, cls in zip(boxes, clss):
            draw_bounding_box(im0, box, cls, color=colors(int(cls), True))
            box_center = (int((box[0] + box[2]) / 2), int((box[1] + box[3]) / 2))
            real_x, real_y = estimate_position(box_center, intrinsic_camera, distortion)
            cv2.putText(im0, f"Real X: {real_x:.2f} mm, Real Y: {real_y:.2f} mm", (box_center[0] + 10, box_center[1] + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
