...
# Load the rotation and translation vectors from calibration results
with open('T2C_PickAndPlace/Data/calReWithRvecTvec/rvecs.pkl', 'rb') as f:
    rvecs = pickle.load(f)
with open('T2C_PickAndPlace/Data/calReWithRvecTvec/tvecs.pkl', 'rb') as f:
    tvecs = pickle.load(f)
...
def estimate_position(center, matrix_coefficients, distortion_coefficients, rvec, tvec):
    ...
    center_undistorted = np.array(center_undistorted).reshape(-1, 1, 2)  # Ensure correct shape
    world_points, _ = cv2.projectPoints(center_undistorted, np.array(rvec), np.array(tvec), matrix_coefficients, distortion_coefficients)
        ...
        for img_path in glob.glob(image_path):
            ...
            if results[0].masks is not None:
                ...
                for mask, cls in zip(masks, clss):
                    ...
                    # Calculate the inner rectangle (40% smaller)
                    inner_width = rect[1][0] * 0.1
                    inner_height = rect[1][1] * 0.1
                    ...
                    real_x, real_y = estimate_position(box_center, intrinsic_camera, distortion, rvecs[0], tvecs[0])
