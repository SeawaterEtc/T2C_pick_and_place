...
size_of_chessboard_squares_mm = 20  # squares size
...
pickle.dump((cameraMatrix, dist), open( "T2C_PickAndPlace/Data/Calibration_result/calibration.pkl", "wb" ))
pickle.dump(cameraMatrix, open( "T2C_PickAndPlace/Data/Calibration_result/cameraMatrix.pkl", "wb" ))
pickle.dump(dist, open( "T2C_PickAndPlace/Data/Calibration_result/dist.pkl", "wb" ))

############## UNDISTORTION #####################################################

# # Directory where the calibration results are saved
# output_dir = "T2C_PickAndPlace/Data/Calibration_result/Undistorted_Images"

# # Create the directory if it doesn't exist
# if not os.path.exists(output_dir):
#     os.makedirs(output_dir)

# # Loop through all images and undistort
...
#     image_name = os.path.basename(image_path)  # Get the original file name
#     save_path = os.path.join(output_dir, f"undistorted_{image_name}")
    
#     # Save the undistorted image
#     cv.imwrite(save_path, dst)
#     print(f"Saved: {save_path}")

# # Optional: Display the last processed image for confirmation
